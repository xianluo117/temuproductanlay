...
export function findAuthorizedImageAsset(
  ...
  const allowedShopIds = [...new Set(Array.isArray(shopIds) ? shopIds : [shopIds])];
  if (!allowedShopIds.length) return null;
  const shopPlaceholders = allowedShopIds.map(() => "?").join(", ");
  const row = database
    .prepare(
      `SELECT asset.id AS asset_id, asset.file_name
       ...
       WHERE asset.file_name = ? AND (
          EXISTS (
            SELECT 1 FROM y2_inventory_products inventory
            WHERE inventory.image_asset_id = asset.id
          )
          OR EXISTS (
           SELECT 1 FROM products product
           WHERE product.shop_profile_id IN (${shopPlaceholders})
         ...
         OR EXISTS (
           ...
           WHERE spu.shop_profile_id IN (${shopPlaceholders}) AND skc.image_asset_id = asset.id
         )
         OR EXISTS (
           ...
           JOIN product_management_records record ON record.id = link.record_id
           WHERE record.shop_profile_id IN (${shopPlaceholders})
             AND item.image_asset_id = asset.id
         )
       )
       LIMIT 1`,
    )
    .get(fileName, ...allowedShopIds, ...allowedShopIds, ...allowedShopIds) as
