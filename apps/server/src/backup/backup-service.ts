...
function getBackupStats(ownerId?: number): BackupStatsRow {
  const dates = database.prepare(`
    SELECT MIN(data_date) AS earliest_data_date,
      ...
      COUNT(DISTINCT spu) AS product_count
    FROM daily_metrics WHERE (? IS NULL OR owner_id = ?)
  `).get(ownerId ?? null, ownerId ?? null) as Omit<BackupStatsRow, 'import_batch_count'>;
  const batches = database.prepare("SELECT COUNT(*) AS count FROM import_batches WHERE status = 'completed' AND (? IS NULL OR owner_id = ?)").get(ownerId ?? null, ownerId ?? null) as { count: number };
...
function readManifest(zipPath: string): BackupManifest {
  ...
  const raw = JSON.parse(entry.getData().toString('utf8')) as Partial<BackupManifest> & { version?: number };
...
function removeExpiredAutomaticBackups(currentFileName: string): void {
  ...
  for (const item of automatic) {
    if (item.fileName !== current.fileName && item.localDate === current.localDate) {
      fs.rmSync(path.join(paths.backups, item.fileName), { force: true });
    }
  }

  const remaining = listBackups().filter((item) => item.type === 'automatic');
  const retainedDates = [...new Set(remaining.map((item) => item.localDate).sort().reverse())].slice(0, 3);
...
export function listBackups(ownerId?: number): BackupInfo[] {
  return fs.readdirSync(paths.backups, { withFileTypes: true })
    ...
    .flatMap((entry): BackupInfo[] => {
      ...
      try {
        ...
        if (ownerId !== undefined && manifest.ownerId !== ownerId) return [];
...
export function createBackup(type: BackupType = 'manual', now = new Date(), ownerId?: number): BackupInfo {
  database.pragma('wal_checkpoint(TRUNCATE)');
  const stats = getBackupStats(ownerId);
  ...
  const baseName = backupFileName(type, now);
  const fileName = ownerId === undefined ? baseName : baseName.replace('.zip', `-user-${ownerId}.zip`);
  const outputPath = path.join(paths.backups, fileName);
  const temporaryPath = `${outputPath}.tmp`;
  const zip = new AdmZip();
  zip.addFile('manifest.json', Buffer.from(JSON.stringify(manifest, null, 2)));
  if (!fs.existsSync(paths.database)) throw new Error('数据库文件不存在，无法创建备份。');
  zip.addLocalFile(paths.database, 'database');
  if (fs.existsSync(paths.images)) zip.addLocalFolder(paths.images, 'images', (name) => !name.endsWith('.gitkeep'));
  if (fs.existsSync(paths.imports)) zip.addLocalFolder(paths.imports, 'imports', (name) => !name.endsWith('.gitkeep'));
  zip.writeZip(temporaryPath);
  fs.renameSync(temporaryPath, outputPath);
  if (type === 'automatic' && ownerId === undefined) removeExpiredAutomaticBackups(fileName);
...
function resolveStoredBackup(fileName: string): string {
  ...
  if (safeName !== fileName || !safeName.endsWith('.zip')) throw new Error('备份文件名无效。');
...
function validateExtractedDatabase(databasePath: string): void {
  ...
  try {
    ...
    if (result !== 'ok') throw new Error(`数据库完整性校验失败：${String(result)}`);
    const required = candidate.prepare("SELECT COUNT(*) AS count FROM sqlite_master WHERE type = 'table' AND name IN ('products', 'daily_metrics', 'import_batches')").get() as { count: number };
...
export async function restoreBackup(backupPath: string): Promise<BackupManifest> {
  ...
  try {
    ...
    const extractedDatabase = path.join(restoreDirectory, 'database', 'temu-analytics.sqlite');
    ...
    for (const directory of ['images', 'imports'] as const) {
      ...
      if (fs.existsSync(source)) await fsp.cp(source, paths[directory], { recursive: true });
    }
    return manifest;
  } finally {
    await fsp.rm(restoreDirectory, { recursive: true, force: true });
    if (backupPath.startsWith(paths.temp)) await fsp.rm(backupPath, { force: true });
  }
}

export async function restoreStoredBackup(fileName: string): Promise<BackupManifest> {
