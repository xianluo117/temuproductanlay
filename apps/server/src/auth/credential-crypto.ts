...
function encryptionKey(): Buffer {
  if (config.zhihouCredentialKey.trim()) {
    const configuredKey = Buffer.from(config.zhihouCredentialKey.trim(), "base64");
  ...
  if (fs.existsSync(paths.zhihouCredentialKey)) {
    ...
    if (storedKey.length !== 32)
      throw new Error("本地凭据密钥文件无效，请恢复正确密钥或重新配置凭据。");
    return storedKey;
  }
...
export function decryptCredential(encoded: string): string {
  ...
  if (version !== VERSION || !ivValue || !tagValue || ciphertextValue === undefined)
  ...
  try {
    ...
    throw new Error("凭据解密失败，请检查凭据密钥或重新设置密码。");
  }
}
