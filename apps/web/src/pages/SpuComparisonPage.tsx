...
import { Alert, Button, Card, Col, Empty, Image, Row, Select, Space, Spin, Statistic, Tag, Typography } from 'antd';
import { CrownOutlined, SwapOutlined, TrophyOutlined } from '@ant-design/icons';
import ReactECharts from 'echarts-for-react';
import { Link } from 'react-router-dom';
import type { DailyMetric, SpuComparisonCandidate, SpuComparisonProduct, SpuComparisonResponse } from '@temu-analytics/shared';
import { errorMessage, getSpuComparison, getSpuComparisonCandidates } from '../api/client';

const { Title, Text } = Typography;
const colors = ['#ff6a2a', '#1677ff', '#12b76a', '#7f56d9', '#f79009'];
const number = new Intl.NumberFormat('zh-CN');
const percent = (value: number | null) => value === null ? '-' : `${(value * 100).toFixed(2)}%`;
type ComparableMetric = 'impressions' | 'clicks' | 'visitors' | 'cartUsers' | 'orders' | 'detailPaymentConversionRate' | 'impressionOrderConversionRate';
...
function metricValue(metric: DailyMetric, key: ComparableMetric): number | null {
  return metric[key];
}
...
export function SpuComparisonPage() {
  const [candidates, setCandidates] = useState<SpuComparisonCandidate[]>([]);
  ...
  useEffect(() => {
    void getSpuComparisonCandidates().then(setCandidates).catch((reason) => setError(errorMessage(reason)));
  }, []);

  const compare = async (requestedDate?: string) => {
    if (selectedSpus.length < 2 || selectedSpus.length > 5) return;
    setLoading(true);
    try {
      const result = await getSpuComparison(selectedSpus, requestedDate);
      setData(result);
  ...
  const productOptions = candidates.map((item) => ({
    value: item.spu,
    label: item.spu,
  }));
  ...
  return <div>
    ...
    <Card bordered={false} className="comparison-control-card">
      <Row gutter={[16, 16]} align="bottom">
        <Col xs={24} xl={15}><Text strong>选择 SPU</Text><Select mode="multiple" allowClear showSearch maxCount={5} value={selectedSpus} options={productOptions} placeholder="请选择 2-5 个 SPU" onChange={(value) => { setSelectedSpus(value); setData(null); setDate(undefined); }} optionFilterProp="label" style={{ width: '100%', marginTop: 8 }} /></Col>
      ...
      <Text type="secondary" className="comparison-help">只展示全部所选 SPU 都有数据的共同日期；首次对比自动选择最新共同日期。</Text>
    </Card>
    ...
    {!loading && !data && <Card bordered={false} className="section-row"><Empty description="请选择 2-5 个 SPU 开始对比" /></Card>}
    {!loading && data && <>
