...
import type { ProductBatchOperationResult, ProductSummary } from '@temu-analytics/shared';
import type { Dayjs } from 'dayjs';
import dayjs from 'dayjs';
import { Link } from 'react-router-dom';
import { createProductOperationsBatch, errorMessage, getDashboard, getProducts } from '../api/client';
...
const paymentMetricOptions = [{ value: 'orders', label: '支付件数（订单量）' }, { value: 'detailPaidBuyers', label: '支付买家数' }, { value: 'detailPaymentConversionRate', label: '商详支付转化率' }, { value: 'impressionOrderConversionRate', label: '曝光订单转化率' }];
...
export function ProductsPage() {
  ...
  const filteredProducts = useMemo(() => products.filter((item) => {
    ...
    const rateMetric = filters.paymentMetric === 'detailPaymentConversionRate' || filters.paymentMetric === 'impressionOrderConversionRate';
